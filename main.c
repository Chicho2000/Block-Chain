...
void imprimir_arbol(ArbolValidacion* arbol) {
    if (arbol == NULL || arbol->heap == NULL) {
        printf("Arbol de validacion no inicializado o vacio.\n");
        return;
    }

    printf("Arbol de validación (estructura de heap):\n");
...
int main() {
    ...
    alta(blockchains, arbol, 0,CANT_BLOCKCHAINS, "Primer mensaje en cadena 0", &lista_primos, &cant_primos_usados, &cant_primos_totales);
    int id_a_modificar = blockchains[0]->id;
    

    alta(blockchains, arbol, 1,CANT_BLOCKCHAINS, "Mensaje 1 en cadena 1", &lista_primos, &cant_primos_usados, &cant_primos_totales);
    alta(blockchains, arbol, 2,CANT_BLOCKCHAINS, "Mensaje 1 en cadena 2", &lista_primos, &cant_primos_usados, &cant_primos_totales);
    alta(blockchains, arbol, 0,CANT_BLOCKCHAINS, "Segundo mensaje en cadena 0", &lista_primos, &cant_primos_usados, &cant_primos_totales);
    ...
    printf("\nModificando nodo con ID %d en una blockchain fuera de rango (5)\n", id_a_modificar);
    ...
    destruir_arbol_validacion(arbolFalso);
